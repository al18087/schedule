...
def register(userID, start_time, end_time, content):
    flag = 0
    cur.execute('SELECT * FROM yotei WHERE userID = %s'
    ...
    if flag == 0:
        cur.execute('INSERT INTO yotei (userID, starttime, endtime, content)'
...
def deleteSchedule(userID, start_time):
    ...
    cur.execute('SELECT content FROM yotei WHERE userID = %s'
        'AND starttime = %s',
        (userID, start_time))
    content = cur.fetchone()[0]
    
    cur.execute('DELETE FROM yotei WHERE userID = %s'
        'AND starttime = %s',
        (userID, start_time))
    conn.commit()
